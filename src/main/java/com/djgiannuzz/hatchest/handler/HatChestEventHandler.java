		...
		if (event.action.equals(Action.RIGHT_CLICK_BLOCK)
			...
			if (event.entityPlayer.getCurrentEquippedItem() == null)
				if (HCUtility.hasPlayerHatChest(event.entityPlayer)) {
					...
					if (Blocks.chest.canPlaceBlockAt(event.world, event.x
							+ mod[0], event.y + mod[1], event.z + mod[2])) {
						event.world.setBlock(event.x + mod[0],
								event.y + mod[1], event.z + mod[2],
								Blocks.chest);
						TileEntityChest chest = HCUtility
								.getTileEntityFromItemStack(event.entityPlayer
										.getCurrentArmor(3));
						event.world.setTileEntity(event.x + mod[0], event.y
								+ mod[1], event.z + mod[2], chest);
						HCUtility.removePlayerHatChest(event.entityPlayer);
					}
				} else {
					TileEntity tileEntity = event.world.getTileEntity(event.x,
							event.y, event.z);
					if (tileEntity != null
							&& tileEntity instanceof TileEntityChest) {
						TileEntityChest chest = (TileEntityChest) tileEntity;
						HCUtility.putChestOnPlayer(event.entityPlayer, chest,
